 ...
 * @since  1.6
 */
class TestModel extends ItemModel
{
	/**
	...
	public function getData()
	{
		if ($this->data === null)
		{
			...
			$temp = (array) \JFactory::getApplication()->getUserState('com_test.edit.profile.data', array());
	...
	{
		// Get the form.
		$form = $this->loadForm('com_test.profile', 'profile', array('control' => 'jform', 'load_data' => $loadData));
		...
		// For com_fields the context is com_test.user
		\JLoader::import('components.com_fields.helpers.fields', JPATH_ADMINISTRATOR);
		\FieldsHelper::prepareForm('com_test.user', $form, $data);
		...
		if ($isUsernameCompliant && !ComponentHelper::getParams('com_test')->get('change_login_name'))
		{
	...
	protected function loadFormData()
	{
		...
		$this->preprocessData('com_test.profile', $data, 'user');
	...
	{
		if (ComponentHelper::getParams('com_test')->get('frontend_userparams'))
		{
	...
	protected function populateState()
	{
		...
		$params = \JFactory::getApplication()->getParams('com_test');

		// Get the user id.
		$userId = \JFactory::getApplication()->getUserState('com_test.edit.profile.id');
	...
	public function save($data)
	{
		...
		if ($this->getState('load_tags', true))
		{
			...
			$user->tags->getTagIds($user->id, 'com_test.user');
		}
