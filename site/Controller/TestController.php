...
class TestController extends BaseController
{
	...
	public function edit()
	{
		...
		$previousId = (int) $app->getUserState('com_test.edit.profile.id');
		...
		if (!empty($cookieLogin))
		{
			...
			$this->setRedirect(\JRoute::_('index.php?option=com_test&view=login', false));

			return false;
		}

		// Set the user id for the user to edit in the session.
		$app->setUserState('com_test.edit.profile.id', $userId);
		...
		$this->setRedirect(\JRoute::_('index.php?option=com_test&view=profile&layout=edit', false));
	...
	public function save()
	{
		...
		if ($data === false)
		{
			...
			$app->setUserState('com_test.edit.profile.data', $requestData);

			// Redirect back to the edit screen.
			$userId = (int) $app->getUserState('com_test.edit.profile.id');
			$this->setRedirect(\JRoute::_('index.php?option=com_test&view=profile&layout=edit&user_id=' . $userId, false));
		...
		{
			// Save the data in the session.
			$app->setUserState('com_test.edit.profile.data', $data);
		...
		switch ($this->getTask())
		{
			case 'apply':
				...
				$app->setUserState('com_test.edit.profile.id', $return);
				$model->checkout($return);

				// Redirect back to the edit screen.
				$this->setMessage(\JText::_('COM_USERS_PROFILE_SAVE_SUCCESS'));

				$redirect = $app->getUserState('com_test.edit.profile.redirect');
				...
				if (!$redirect)
				{
					$redirect = 'index.php?option=com_test&view=profile&layout=edit&hidemainmenu=1';
				}
			...
			default:
				...
				$app->setUserState('com_test.edit.profile.id', null);
				...
				if (!$redirect)
				{
					$redirect = 'index.php?option=com_test&view=profile&user_id=' . $return;
				}
		...
		$app->setUserState('com_test.edit.profile.data', null);
	}
	...
	protected function postSaveHook(BaseDatabaseModel $model, $validData = array())
	{
		...
		if ($tags)
		{
			...
			$item->tags->getTagIds($item->id, 'com_test.user');
